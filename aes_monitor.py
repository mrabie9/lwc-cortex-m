...
def start_board():
	...
	prog_log = subprocess.run("./openocd.sh", shell=True, capture_output=True, text=True)
...
register_log_full  = open("logs/"+ str(algorithm) + "_" + str(sn) + "_log_" + str(datetime.now()).replace(" ", "__").replace(":", "-") + "_full.txt", "w")
register_log_clean = open("logs/"+ str(algorithm) + "_" + str(sn) + "_log_" + str(datetime.now()).replace(" ", "__").replace(":", "-") + "_clean.txt", "w")
